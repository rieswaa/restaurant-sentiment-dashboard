...
with tab6:
    ...
    randomize = st.button("🔁 Acak Review")
    seed = random.randint(0, 10000) if randomize else 42
    ...
    with col_pos:
        ...
        if len(pos_reviews) >= 3:
            st.write(pos_reviews.sample(3, random_state=seed).tolist())
    ...
    with col_neg:
        ...
        if len(neg_reviews) >= 3:
            st.write(neg_reviews.sample(3, random_state=seed).tolist())
