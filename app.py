...
ax1.set_facecolor('#0e1117')         # Background kotak grafik
fig1.patch.set_facecolor('#0e1117')  # Background luar grafik
st.pyplot(fig1)
...
with col_pos:
    st.markdown("**✅ Positif**")
    st.write(filtered_df[filtered_df['Sentiment'] == 'Positive']['Review'].sample(3, random_state=1).tolist())
with col_neg:
    st.markdown("**❌ Negatif**")
    st.write(filtered_df[filtered_df['Sentiment'] == 'Negative']['Review'].sample(3, random_state=1).tolist())
