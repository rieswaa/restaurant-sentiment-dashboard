...
with tab6:
    ...
    if st.button("🔁 Acak Review", key="acak_review"):
    ...
    with col_pos:
        ...
        if len(pos_reviews) >= 3:
            st.write(pos_reviews.sample(3, random_state=random_seed).tolist())
    ...
    with col_neg:
        ...
        if len(neg_reviews) >= 3:
            st.write(neg_reviews.sample(3, random_state=random_seed).tolist())
